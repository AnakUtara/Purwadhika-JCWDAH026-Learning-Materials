...
function App() {
	...
	useEffect(() => {
		...
		const handleMessage = (data: ChatMessage & { id: string }) => {
			// Hanya update state jika event message-received berhasil didengar
			setChatMessages((prev) => [...prev, data]);
		};

		const handleQueueUpdate = (data: { message: string }) => {
			console.log("Queue update received: ", data);
			toast.success(data.message, { id: "queue-update" });
		};
		...
		return () => {
			...
			// Memastikan message nanti tidak terduplikasi
			// karena kedua browser mendengarkan event yang sama dari backend
